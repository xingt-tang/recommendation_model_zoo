...
#coding=utf-8
"""
Implementation of Deep Factorization Machines
"""
...
#model config
tf.app.flags.DEFINE_string("keep_prob",'0.5,0.6',"keep probability for bilinear layer and hidden layer(1-dropout_ratio)")
...
def deepfm_fn(features,labels,mode,params):
    ...
    assert len(keep_prob) == len(hidden_layer)

    learning_rate = params["learning_rate"]
    l2_reg= tf.contrib.layers.l2_regularizer(params["l2_reg"])

    #featur embedding
    feat_embedding = tf.get_variable("feature_embedding",shape=[feat_size,embd_size],initializer=tf.glorot_normal_initializer())
    feat_bias = tf.get_variable("feature_bias",shape=[feat_size,1],initializer=tf.glorot_normal_initializer())
    ...
    with tf.variable_scope("fm"):
        ...
        fm_out = tf.add_n([fm_out,linear_out])

    with tf.variable_scope("deep"):
        deep_out = tf.reshape(emb, shape=[None, field_size*embd_size])
        for i in range(len(hidden_layer)):
            deep_out = tf.contrib.layers.fully_connected(inputs=deep_out, num_outputs=hidden_layer[i],weights_regularizer=l2_reg, scope='mlp_%d' % i)
        ...
        deep_out = tf.contrib.layers.fully_connected(inputs=deep_out, num_outputs = 1, activation_fn=tf.identity,weights_regularizer = l2_reg, scope="prediction")
    ...
    with tf.variable_scope("deepfm_out"):
        ...
        logits = tf.add_n([bias,deep_out,fm_out])
...
def main(_):
    ...
    DEEPFM= tf.estimator.Estimator(model_fn=deepfm_fn, model_dir=model_dir, params=model_params, config=config)

    if FLAGS.task_type == 'train':
        DEEPFM.train(input_fn=lambda: input_fn(tr_file, num_epochs=FLAGS.num_epochs, batch_size=FLAGS.batch_size))
        ...
        DEEPFM.evaluate(input_fn=lambda: input_fn(te_file, num_epochs=1, batch_size=FLAGS.batch_size))
        ...
        DEEPFM.export_savedmodel(FLAGS.out_dir, serving_input_receiver_fn)
